...
def scangff(fp):
    ...
    while line:
        ...
        if len(tabdelim[8]) > 0:
            attribs = tabdelim[8].rstrip(';').split(';')
            rnames = [shlex.split(x)[0] for x in attribs]
    ...
    while line:
        ...
        if len(tabdelim[8]) > 0:
            ...
            for attrib in attribs:
                namevalue = shlex.split(attrib)
                if namevalue[0] in attl:
                    lines[namevalue[0]] += [namevalue[1]]
                    attl.remove(namevalue[0])
                else: lines[namevalue[0]][len(lines[namevalue[0]]) - 1] += ';' + namevalue[1]
