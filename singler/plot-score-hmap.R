...
plot_score_heatmap <- function(
    results, cells.use = NULL, labels.use = NULL,
...
.plot_score_heatmap <- function(
  ...
  args <- list(
    border_color = NA, show_colnames = show_colnames,
...
.trim_normalize_reorder_scores <- function(
  ...
  if (!is.null(rows.order)) {
    if (any(!colnames(scores) %in% rows.order)) {
      ...
      warning(
        "Label(s) of ", scores.title, " missing from 'rows.order' will not be plotted: ",
...
.trim_byLabel_and_normalize_scores <- function(
  ...
  if (!any(is.na(scores))) {
    ...
    secondary <- DelayedArray::rowMaxs(scale(t(scores)), na.rm = TRUE)
  } else {
    # If there are NAs - usually from combine_recomputed -
  ...
  if (normalize) {
    if (ncol(scores) > 1L) {
      mmax <- DelayedArray::rowMaxs(scores, na.rm = TRUE)
      mmin <- DelayedArray::rowMins(scores, na.rm = TRUE)
