...
def qc(fname, run_id, qclog, run_scrublet = False):
    ...
    with open(qclog, 'w') as qclogf:
        with redirect_stdout(qclogf):
            ...
            for s in samples:
                ...
                splice.obs['pass.qc.umi'] = ov.pp._qc.mads_test(splice.obs, 'n.umi', nmads = 5, lt = thresholds)
                splice.obs['pass.qc.detection'] = ov.pp._qc.mads_test(splice.obs, 'n.det', nmads = 5, lt = thresholds)  

                counts_thr = ov.pp._qc.mads(splice.obs, 'n.umi', nmads = 5, lt = thresholds)
                dets_thr = ov.pp._qc.mads(splice.obs, 'n.det', nmads = 5, lt = thresholds)
                ...
                fig1, a1 = plt.subplots(figsize = (3, 3), dpi = 72)
                sc.pl.scatter(splice, x = 'n.umi', y = 'pct.mito', color = 'is.doublet', palette = ['k', 'r'], ax = a1, show = False)
                a1.hlines(
                    y = [0, thresholds['mt.up']], 
                    xmin = 0, xmax = max(max(splice.obs['n.umi']), counts_thr[1]), 
                    color = 'red', ls = 'dashed'
                )
                a1.vlines(
                    ymin = 0, ymax = max(max(splice.obs['pct.mito']), thresholds['mt.up']), 
                    x = counts_thr, 
                    color = 'red', ls = 'dashed'
                )
                ...
                a1.xaxis.set_major_formatter(fformat(format_tick_labels))
                plt.margins(x = 0.2, y = 0.2)

                # fig1.tight_layout()
                fig1.savefig('processed/graphs/mito/{0}.{1}.png'.format(name, s))

                fig2, a2 = plt.subplots(figsize = (3, 3), dpi = 72)
                sc.pl.scatter(splice, x = 'n.umi', y = 'n.det', color = 'is.doublet', palette = ['k', 'r'], ax = a2, show = False)
                a2.hlines(
                    y = dets_thr, 
                    xmin = 0, xmax = max(max(adata.obs['n.umi']), counts_thr[1]), 
                    color = 'red', ls = 'dashed'
                )
                a2.vlines(
                    ymin = 0, ymax = max(max(splice.obs['n.det']), dets_thr[1]), 
                    x = counts_thr, 
                    color = 'red', ls = 'dashed'
                )
                ...
                plt.yticks(rotation = 90)
                # plt.xlabel("UMI count")
                # plt.ylabel("Gene detection")
                plt.xlabel(''); plt.ylabel('')
                a2.yaxis.set_major_formatter(fformat(format_tick_labels))
                a2.xaxis.set_major_formatter(fformat(format_tick_labels))
                plt.margins(x = 0.2, y = 0.2)

                # fig2.tight_layout()
                fig2.savefig('processed/graphs/detections/{0}.{1}.png'.format(name, s))
            ...
            merged.write_h5ad('processed/metrics/{0}.h5ad'.format(name))
